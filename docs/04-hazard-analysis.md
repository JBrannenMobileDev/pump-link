...
| H-12 | The phone loses the `CommandId` for a command that may be in progress. Permanently unresolvable. | Critical | Process death after transmitting but before journaling | Journal is committed and flushed before transmission; a `connectedDevice` foreground service holds the process at least while a command is in flight (REQ-S-02). The shipping hold is the requested session, which is a stricter case of the same rule. | `SessionKeepAliveTest`; process-death instrumented case |
